 ...
 *   int camera_id: ID of the camera
 * \param[in]
 *   int vc_num: the total virtual channel camera number, default value is 0
 *
 * \return
 ...
 * \param[in]
 *   int input_fmt: (Optional) Specify which input format(the output of ISYS) should be used.
 *
 * \return
 ...
 * \endcode
 **/
int camera_device_config_streams(int camera_id, stream_config_t *stream_list, int input_fmt = -1);
...
/**
 * \brief
 *   Dequeue a buffer from device
 *
 * \note
 *   Dqueue buffer is block function, that means dqbuf will block until buffer is ready.
 *
 * \param[in]
 *   int camera_id: ID of the camera
 * \param[in]
 ...
 *   int buffer_length = CameraUtils::getFrameSize(V4L2_PIX_FMT_SGRBG8, 1920, 1080);
 *   camera_buffer_t *buffers = (camera_buffer_t *)malloc(sizeof(camera_buffer_t)*buffer_count);
 *   camera_buffer_t *buf = buffers;
 *   for (int i = 0; i < buffer_count; i++, buf++) {
 *     posix_memalign(&buf->addr, getpagesize(), buffer_length);
 *     buffer->width = 1920;
 *     buffer->height = 1080;
 *     buffer->format = V4L2_PIX_FMT_SGRBG8;
 *   }
 *
 *   buf = buffers;
 *   for (int i = 0; i < buffer_count; i++, buf++) {
...
/**
 * \brief
 *   Set parameter to specific camera device.
 *
 * \note
 ...
 *   int camera_id: ID of the camera
 * \param[in]
 *   Parameters param:  parameters need to be set
 *
 * \return
...
/**
 * \brief
 *   Get parameter from specific camera device.
 *
 * \note
 *   It MUST be called after device opened, otherwise error will be returned.
 *

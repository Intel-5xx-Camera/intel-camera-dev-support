...
extern camera_module_t HAL_MODULE_INFO_SYM;  // For 'HMI'
...
TEST_P(SupportedStreamsTest, TestStream)
{
    ...
    ASSERT_LT(param.cameraId, HAL_MODULE_INFO_SYM.get_number_of_cameras());

    // get the static metadata, which has available stream configs
    struct camera_info ac2info;
    HAL_MODULE_INFO_SYM.get_camera_info(param.cameraId, &ac2info);
...
INSTANTIATE_TEST_CASE_P(imx230,
                        SupportedStreamsTest,
                        ::testing::ValuesIn(Parmz::getSupportedStreams(&TSF::getImx230SupportedStreams, 0)));

INSTANTIATE_TEST_CASE_P(imx214,
                        SupportedStreamsTest,
                        ::testing::ValuesIn(Parmz::getSupportedStreams(&TSF::getImx214SupportedStreams, 0)));

INSTANTIATE_TEST_CASE_P(ov5670,
                        SupportedStreamsTest,
                        ::testing::ValuesIn(Parmz::getSupportedStreams(&TSF::getOv5670SupportedStreams, 1)));

INSTANTIATE_TEST_CASE_P(ov8858,
                        SupportedStreamsTest,
                        ::testing::ValuesIn(Parmz::getSupportedStreams(&TSF::getOv8858SupportedStreams, 0)));

INSTANTIATE_TEST_CASE_P(imx135,
                        SupportedStreamsTest,
                        ::testing::ValuesIn(Parmz::getSupportedStreams(&TSF::getImx135SupportedStreams, 0)));
